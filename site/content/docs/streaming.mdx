...
Earl strips the `data: ` prefix from each line before decoding. With `decode = "json"`, each event that parses as JSON produces a `result` — non-JSON lines like `[DONE]` are skipped automatically.
